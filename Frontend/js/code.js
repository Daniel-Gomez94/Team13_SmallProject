...
function searchContacts()
{
	...
	// show the contact list area when searching
...
function displayContacts(contacts)
{
	...
	if(contacts.length === 0) {
		...
		for(let i = 0; i < contacts.length; i++) {
			...
			contactHtml += "<div class='contact-actions'>";
			contactHtml += "<button class='edit-btn' onclick='openEditModal(" + contact.id + ", \"" + contact.firstName + "\", \"" + contact.lastName + "\", \"" + (contact.email || "") + "\", \"" + (contact.phone || "") + "\")'>Edit</button>";
...
function deleteContact(contactId)
{
	...
	try
	{
		xhr.onreadystatechange = function() 
		{
			...
			{
				// only refresh if contacts are currently visible
				if (contactsVisible || document.getElementById("searchText").value.trim()) {
					loadContacts();
				}
			}
			else if (this.readyState == 4)
...
function openEditModal(contactId, firstName, lastName, email, phone)
{
	...
	document.getElementById("editFirstNameText").value = firstName;
	document.getElementById("editLastNameText").value = lastName;
	...
	try
	{
		xhr.onreadystatechange = function() 
		{
			if (this.readyState == 4 && this.status == 200) 
			{
				...
				setTimeout(() => {
					closeEditModal();
					// refresh contacts if they're currently visible
					if (contactsVisible || document.getElementById("searchText").value.trim()) {
						loadContacts();
					}
				}, 1500);
			...
			{
				let jsonObject = JSON.parse(xhr.responseText);
				document.getElementById("contactUpdateResult").innerHTML = jsonObject.error || "Error updating contact";
			}
		};
		xhr.send(jsonPayload);
	}
